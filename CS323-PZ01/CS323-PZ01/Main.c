...
Player* readAllPlayers(int* size) {
	...
	if (!playersFile) {
		perror("Greska pri otvaranju fajla players.txt");
		exit(1);
	}
...
void writePlayers(Player player) {
	FILE* playersFile = fopen("players.txt", "w");
...
int* readAwards() {
	FILE* awardsFile = fopen("awards.txt", "r");

	if (awardsFile == NULL) {
		fprintf(stderr, "Greska pri otvaranju datoteke 'awards.txt'\n");
		exit(1);
	}
...
QuestionAnswer* readAllSetsOfQuestions(int* size) {
	FILE* questionsFile = fopen("questionsAnswers.txt", "r");

	if (!questionsFile) {
		perror("Greska pri otvaranju fajla questionsAnswers.txt");
		exit(1);
	}
